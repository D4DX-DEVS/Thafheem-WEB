...
const BookVerse = () => {
  ...
  const [surahNameMaps, setSurahNameMaps] = useState({});
  ...
  useEffect(() => {
    const loadBookmarks = async () => {
      ...
      try {
        ...
        // Format bookmarks for display (preserve language and stored surah name from API)
        const formattedBookmarks = bookmarks.map(bookmark => ({
          id: bookmark.id || `${bookmark.surahId}-${bookmark.verseId}`,
          bookmarkId: bookmark.id,
          number: bookmark.surahId,
          // Use the surah name stored at bookmark time - it's already in the right language
          surah: bookmark.surahName || '',
          ayah: `Ayah ${bookmark.verseId}`,
          verseText: bookmark.verseText || '',
          surahId: bookmark.surahId,
          verseId: bookmark.verseId,
          language: bookmark.language || translationLanguage || 'en',
        }));

        setBookmarkedVerses(formattedBookmarks);

        // Only fetch surah names for bookmarks that have no stored name (old bookmarks)
        ...
        if (langsNeeded.length > 0) {
          const maps = {};
          await Promise.all(
            langsNeeded.map(lang =>
              fetchSurahs({ language: lang })
                .then(surahs => {
                  const map = {};
                  surahs.forEach(s => { map[s.number] = s.name; });
                  maps[lang] = map;
                })
                .catch(() => { maps[lang] = {}; })
            )
          );
          setSurahNameMaps(maps);
        }
      } catch (err) {
        ...
        <div className="w-full max-w-[1290px] mx-auto">
          ...
          <div className="grid grid-cols-1 sm:grid-cols-1 md:grid-cols-2 lg:grid-cols-3 xl:grid-cols-3 gap-3 sm:gap-4 lg:gap-6">
            {bookmarkedVerses.map((verse) => {
              const verseLang = verse.language || 'en';
              const fontClass = LANGUAGE_FONT_CLASS[verseLang] || 'font-poppins';
              const langLabel = LANGUAGE_LABELS[verseLang] || verseLang.toUpperCase();
              // Use the stored surah name (already in correct language),
              // fall back to re-fetched map only for old bookmarks that have no stored name,
              // never fall back to a different language's map.
              const langMap = surahNameMaps[verseLang] || {};
              const displayName = verse.surah || langMap[verse.surahId] || `Surah ${verse.surahId}`;
              ...
              return (
                ...
                >
                  ...
                  <div className="flex items-center flex-1 min-w-0 gap-2">
                    <span
                      className="flex-shrink-0 text-[10px] font-semibold px-1.5 py-0.5 rounded bg-cyan-100 dark:bg-cyan-900 text-cyan-700 dark:text-cyan-300 uppercase tracking-wide"
                      title={`Bookmarked in ${verseLang}`}
                    >
                      {langLabel}
                    </span>
                    <div className="min-w-0 flex-1">
                      <h3 className={`text-sm sm:text-base font-medium text-gray-900 dark:text-white truncate ${fontClass}`}>
                        {displayName}
                      </h3>
