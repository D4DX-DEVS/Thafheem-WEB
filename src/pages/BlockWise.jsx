...
const BlockWise = () => {
  ...
  const [audioTypes, setAudioTypes] = useState(['quran']); // BlockWise stays quran-only
  ...
  const pendingBlocksRef = useRef([]);   // background blocks waiting for viewport
  const blockProcessorRef = useRef(null); // processWithThrottle fn — set during load
  ...
  // Save reading progress when visiting blockwise page
  useEffect(() => {
    if (surahId) {
      saveLastReading({
        surahId,
  ...
  // Track which block is visible and save position for "continue reading"
  useEffect(() => {
    if (!surahId) return;
    let saveTimer = null;
    ...
    const handleScroll = () => {
      ...
      saveTimer = setTimeout(() => {
        ...
        blockEls.forEach((el) => {
          ...
          // Pick the last block whose top is at or above the middle of the viewport
          if (rect.top <= window.innerHeight / 2 && rect.bottom > 0) {
            bestEl = el;
          }
        });
        if (bestEl) {
          const rangeKey = bestEl.id.replace(/^block-/, '');
          saveLastReading({
            surahId,
            viewType: "blockwise",
            path: `/blockwise/${surahId}`,
            scrollToBlock: rangeKey,
          });
        }
      }, 600);
    };
  ...
  const generateAudioUrl = async (surahId, ayahId, type, interpretationNumber = 1) => {
    ...
    if (type === 'quran') {
      return { primary: `${baseUrl}/qirath/${selectedQirath}/${qirathPrefixes[selectedQirath]}${surahCode}_${ayahCode}.ogg`, fallbacks: [] };
      ...
      if (translationLanguage === 'mal' && ayahId > 1) {
        ...
        fallbacks.push(`${baseUrl}/translation/T${surahCode}_${previousAyahCode},${ayahCode}.ogg`);
      }
      if (translationLanguage === 'mal') {
        const nextAyahCode = String(ayahId + 1).padStart(3, "0");
        fallbacks.push(`${baseUrl}/translation/T${surahCode}_${ayahCode},${nextAyahCode}.ogg`);
      }
      ...
      // Add fallbacks for grouped ayahs (only for first interpretation)
      const fallbacks = [];
      if (translationLanguage === 'mal' && interpretationNumber === 1) {
        if (ayahId > 1) {
          const previousAyahCode = String(ayahId - 1).padStart(3, "0");
          fallbacks.push(`${baseUrl}/interpretation/I${surahCode}_${previousAyahCode},${ayahCode}.ogg`);
        }
        const nextAyahCode = String(ayahId + 1).padStart(3, "0");
        fallbacks.push(`${baseUrl}/interpretation/I${surahCode}_${ayahCode},${nextAyahCode}.ogg`);
      }
      return { primary: primaryUrl, fallbacks };
    }
  ...
  const playAyahAudioWithTypes = async (blockId, ayahNumber, audioTypeIndex = 0, typesToPlay = null) => {
    ...
    const tryLoadAudio = (urlToTry, isFallback = false) => {
      ...
      audioRef.current.onerror = (error) => {
        ...
        if (isTryingFallbackRef.current && !isFallback) {
        ...
        if (translationLanguage === 'mal' && currentFallbackIndex < fallbacks.length - 1) {
          ...
          console.log(`[BlockWise] ${currentAudioType} audio failed, trying fallback ${currentFallbackIndex + 1}/${fallbacks.length}:`, fallbacks[currentFallbackIndex]);
        ...
        if (isFallback && translationLanguage === 'mal') {
          console.log(`[BlockWise] All fallbacks exhausted for ${currentAudioType} audio, moving to next audio type`);
          isTryingFallbackRef.current = false;
        }
      ...
      // Preload next audio file while current one is playing
      audioRef.current.addEventListener('canplay', () => {
        // Determine what comes next and preload it
        const nextTypeIndex = audioTypeIndex + 1;
        if (nextTypeIndex < activeAudioTypes.length) {
          // Preload next audio type for same ayah
          generateAudioUrl(surahId, ayahToPlay, activeAudioTypes[nextTypeIndex]).then(urls => {
            if (urls?.primary) audioManager.preload(urls.primary);
          }).catch(() => {});
        } else {
          // Preload first audio type for next ayah
          const nextAyah = ayahToPlay + 1;
          generateAudioUrl(surahId, nextAyah, activeAudioTypes[0]).then(urls => {
            if (urls?.primary) audioManager.preload(urls.primary);
          }).catch(() => {});
        }
      }, { once: true });
        ...
        loadTimeout = setTimeout(() => {
          if (audioRef.current && audioRef.current.readyState < 2 && 
            ...
            if (translationLanguage === 'mal' && currentFallbackIndex < fallbacks.length - 1) {
              currentFallbackIndex++;
              isTryingFallbackRef.current = true;
              console.log(`[BlockWise] Audio timeout, trying fallback ${currentFallbackIndex + 1}/${fallbacks.length}:`, fallbacks[currentFallbackIndex]);
              tryLoadAudio(fallbacks[currentFallbackIndex], true);
              return;
            }
      ...
      audioRef.current.play().catch(err => {
        ...
        if (translationLanguage === 'mal' && currentFallbackIndex < fallbacks.length - 1) {
          currentFallbackIndex++;
          isTryingFallbackRef.current = true;
          console.log(`[BlockWise] ${currentAudioType} audio play failed, trying fallback ${currentFallbackIndex + 1}/${fallbacks.length}:`, fallbacks[currentFallbackIndex]);
          // Reset audio element before trying fallback
          if (audioRef.current) {
            try {
              audioRef.current.pause();
  ...
  useEffect(() => {
    const loadBlockWiseData = async () => {
      ...
      try {
        ...
        const [ayaRangesResult, arabicResult] = await Promise.allSettled([
          fetchAyaRanges(parseInt(surahId), translationLanguage || 'mal'),
          fetchArabicVerses(parseInt(surahId)),
        ]);

        const ayaRangesResponse = ayaRangesResult.status === 'fulfilled' ? ayaRangesResult.value : null;
        const arabicResponse = arabicResult.status === 'fulfilled' ? arabicResult.value : null;

        if (ayaRangesResult.status === 'rejected') {
          throw ayaRangesResult.reason; // Aya ranges are critical — rethrow
        }
        if (arabicResult.status === 'rejected') {
          console.error('Error fetching Arabic verses:', arabicResult.reason?.message);
        }
        ...
        if (ayaRangesResponse && ayaRangesResponse.length > 0) {
          ...
          // REQUEST THROTTLING: Keep concurrency low to avoid saturating the MySQL connection pool (10 connections).
          // Visible blocks (first 3) already run unthrottled in parallel; background blocks stay at 2 concurrent.
          const INITIAL_CONCURRENT = 2;
          const MAX_CONCURRENT = 2;  // Do not increase — visible blocks already add 3 unthrottled requests
          const SUCCESS_THRESHOLD = 999; // Never trigger the increase

          let MAX_CONCURRENT_REQUESTS = INITIAL_CONCURRENT;
              ...
              const execute = async () => {
                ...
                try {
                  ...
                  if (successfulLoads === SUCCESS_THRESHOLD && MAX_CONCURRENT_REQUESTS < MAX_CONCURRENT) {
                    ...
                    // Concurrent requests increased after successful loads
                  }
                } catch (error) {
          ...
          // First 3 blocks load immediately (above the fold).
          // All remaining blocks are stored in pendingBlocksRef and loaded
          // by IntersectionObserver ONLY when they scroll near the viewport.
          // This keeps the DB pool free for visible content.
          const visibleBlocks    = ayaRangesResponse.slice(0, 3);
          const backgroundBlocks = ayaRangesResponse.slice(3);

          // Expose throttled processor to the observer effect
          blockProcessorRef.current = processWithThrottle;
          pendingBlocksRef.current  = backgroundBlocks;

          // Load first 3 blocks in parallel (no throttle)
          visibleBlocks.forEach((block, index) => processBlockTranslation(block, index));
        }
      } catch (err) {
    ...
    // Reset refs when surahId/language changes
    return () => {
      hasFetchedRef.current = false;
      pendingBlocksRef.current  = [];
      blockProcessorRef.current = null;
    };
  }, [surahId, surahs, translationLanguage, blockReloadToken]);

  // ── IntersectionObserver: load background blocks when they near the viewport ──
  // Runs after blockRanges state update (DOM blocks exist at this point).
  useEffect(() => {
    if (blockRanges.length <= 3 || pendingBlocksRef.current.length === 0) return;

    const observer = new IntersectionObserver(
      (entries) => {
        for (const entry of entries) {
          if (!entry.isIntersecting) continue;
          const pendingIdx = parseInt(entry.target.dataset.pendingIdx, 10);
          const block = pendingBlocksRef.current[pendingIdx];
          if (block && blockProcessorRef.current) {
            blockProcessorRef.current(block, pendingIdx + 3);
            pendingBlocksRef.current[pendingIdx] = null; // mark consumed
          }
    ...
    pendingBlocksRef.current.forEach((block, idx) => {
      if (!block) return;
      const start = block.AyaFrom ?? block.ayafrom ?? block.from;
      const end   = block.AyaTo   ?? block.ayato   ?? block.to;
      const el    = document.getElementById(`block-${start}-${end}`);
      if (el) {
        el.dataset.pendingIdx = String(idx);
        observer.observe(el);
      }
    });
  ...
  // ── Priority-load handler: when user selects a block from the dropdown ──
  // Bypasses the IntersectionObserver queue so the target block loads immediately
  useEffect(() => {
    if (blockRanges.length === 0) return;

    const handlePriorityLoad = (event) => {
      const { rangeKey } = event.detail || {};
      if (!rangeKey) return;

      // Find the block that matches this rangeKey
      const blockIndex = blockRanges.findIndex((block) => {
        const rawStart = block.AyaFrom ?? block.ayafrom ?? block.from ?? 1;
        const rawEnd = block.AyaTo ?? block.ayato ?? block.to ?? rawStart;
        const parsedStart = Number.parseInt(rawStart, 10);
        const parsedEnd = Number.parseInt(rawEnd, 10);
        const hasNumericBounds = Number.isFinite(parsedStart) && Number.isFinite(parsedEnd);
        const start = hasNumericBounds ? parsedStart : (Number.isFinite(Number(rawStart)) ? Number(rawStart) : 1);
        const end = hasNumericBounds ? parsedEnd : (Number.isFinite(Number(rawEnd)) ? Number(rawEnd) : start);
      ...
      const rawAyaFrom = block.AyaFrom || block.ayafrom || block.from;
      const rawAyaTo = block.AyaTo || block.ayato || block.to;
      if (rawAyaFrom == null || rawAyaTo == null) return;

      const normalizedFrom = Number.parseInt(rawAyaFrom, 10);
      const normalizedTo = Number.parseInt(rawAyaTo, 10);
      const hasNumericRange = Number.isFinite(normalizedFrom) && Number.isFinite(normalizedTo);
      const range = hasNumericRange ? `${normalizedFrom}-${normalizedTo}` : `${rawAyaFrom}-${rawAyaTo}`;
      const blockId = block.ID || block.id || range || `block-${blockIndex}`;

      // Skip if already loaded
      if (blockTranslations[blockId]?.data) return;

      // Remove from processedBlocks so it can be retried if a previous attempt failed
      processedBlocksRef.current.delete(blockId);

      // Also remove from pending blocks to avoid duplicate loads from observer
      if (blockIndex >= 3) {
        const pendingIdx = blockIndex - 3;
        if (pendingBlocksRef.current[pendingIdx]) {
          pendingBlocksRef.current[pendingIdx] = null;
        }
      }

      // Directly load this block's translation (bypass throttle queue)
      const loadPriorityBlock = async () => {
        if (processedBlocksRef.current.has(blockId)) return;
        processedBlocksRef.current.add(blockId);
        ...
        try {
          ...
          setBlockTranslations(prev => {
            if (prev[blockId]?.data) return prev;
            return {
              ...prev,
              [blockId]: {
                range,
                ayaFrom: hasNumericRange ? normalizedFrom : rawAyaFrom,
                ayaTo: hasNumericRange ? normalizedTo : rawAyaTo,
                data: translationData,
              }
            };
          });

          // After translation loads, the block's DOM height changes.
          // Dispatch re-scroll event so Transition can re-position to this block.
          requestAnimationFrame(() => {
            window.dispatchEvent(new CustomEvent('blockwise-rescroll', { detail: { rangeKey } }));
          });
        } catch (error) {
          console.error(`Failed to priority-load block ${range}:`, error.message);
        } finally {
          setLoadingBlocks(prev => {
            const updated = new Set(prev);
            updated.delete(blockId);
    ...
    window.addEventListener('blockwise-priority-load', handlePriorityLoad);
    return () => window.removeEventListener('blockwise-priority-load', handlePriorityLoad);
  }, [blockRanges, blockTranslations, surahId, translationLanguage]);
  ...
  const handleNumberClick = (number) => {
    // Analytics — blockwise interpretation popup open.
    if (analytics) {
  ...
  const parseTranslationWithClickableSup = (htmlContent, blockRange) => {
    ...
    // For Hindi, match numbers followed by A or B (e.g. 25A, 18A, 20A, 27B)
  ...
  // Handle scroll to specific block when navigating from bookmark or verse reference
  useEffect(() => {
    const scrollToBlockFromState = () => {
      ...
      if (scrollToBlockRange) {
        targetBlockId = `block-${scrollToBlockRange}`;
      } else if (scrollToVerse && blockRanges.length > 0) {
        // Find which block contains this verse
        const block = blockRanges.find(b => {
          const from = parseInt(b.AyaFrom || b.ayafrom || b.from || 0, 10);
          const to = parseInt(b.AyaTo || b.ayato || b.to || from, 10);
          return scrollToVerse >= from && scrollToVerse <= to;
        });
        if (block) {
          const blockFrom = block.AyaFrom || block.ayafrom || block.from || 1;
          const blockTo = block.AyaTo || block.ayato || block.to || blockFrom;
          targetBlockId = `block-${blockFrom}-${blockTo}`;
        }
      }

      if (targetBlockId && !loading && blockRanges.length > 0) {
                      ...
                      <div className="w-full mb-6 sm:mb-8 text-right" dir="rtl">
                        <p
                          ...
                          style={{
                            ...
                            direction: 'rtl',
                            lineHeight: tajweedEnabled ? '1.8' : '2.7',
                        ...
                        >
                          {arabicSlice.length > 0
                            ? arabicSlice.map((verse, idx) => {
                                const ayahNum = start + idx;
                                return (
                                  <React.Fragment key={`verse-${ayahNum}`}>
                                    {tajweedEnabled ? (
                                      <>
                                        <TajweedVerse suraid={surahId} ayaid={ayahNum} fontSize={26} />
                                        <span style={{ fontSize: `${Math.round(fontSize * 0.75)}px`, verticalAlign: 'middle' }}>{`\u00A0﴿${toArabicNumber(ayahNum)}﴾\u00A0`}</span>
                                      </>
                                    ) : (
                                      <>
                                        <HoverableArabicText
                                          arabicText={stripArabicVerseMarker(verse.text_uthmani)}
                                          surahId={surahId}
                                          verseNumber={ayahNum}
                                          language={translationLanguage}
                                          fontSize={fontSize}
            ...
            <div className="max-w-4xl mx-auto">
              ...
              <div className="hidden sm:flex items-center justify-center space-x-2 sm:space-x-4 lg:space-x-6">
                ...
                <button
                  ...
                  className="flex items-center space-x-2 px-3 sm:px-4 py-2 text-xs sm:text-sm bg-white border border-gray-300 rounded-full shadow-sm text-gray-600 dark:bg-[#252525] dark:text-white hover:text-gray-900 dark:hover:text-gray-300 transition-colors min-h-[44px] sm:min-h-[48px]"
                >
