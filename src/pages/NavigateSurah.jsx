...
const NavigateSurah = ({ onClose, onSurahSelect }) => {
  ...
  const getTabLabel = (tab) => tabTranslations[translationLanguage]?.[tab] || tabTranslations.E[tab] || tab;

  // Reset outer scroll position when switching tabs
  useEffect(() => {
  ...
  const NavigateSurahList = () => {
    ...
    // Rendered via portal outside route context — useParams is empty there,
    // so fall back to parsing the surah id from the pathname.
    const { surahId: routeSurahId } = useParams();
    const location = useLocation();
    const currentSurahId =
      routeSurahId ||
      location.pathname.match(/^\/(?:surah|reading|blockwise|surahinfo|word-by-word)\/(\d+)/)?.[1];
    const selectedItemRef = useRef(null);

    // Scroll to the currently selected surah when the list mounts.
    // rAF so it runs after the parent's scrollTop reset effect; [loading]
    // so it re-runs once surah data arrives and the ref is attached.
    useEffect(() => {
    ...
    return (
      <div className="bg-white dark:bg-[#1C1C1E] flex flex-col h-full font-poppins">
        {/* Search Bar */}
                ...
                return (
                  <li
                    ...
                    className={`cursor-pointer transition-colors py-1.5 px-2 rounded-lg ${
                      String(surah.id || surah.number) === String(currentSurahId)
                        ? 'text-primary bg-cyan-50 dark:bg-cyan-900/20 font-medium'
                        : 'text-gray-800 dark:text-white hover:text-primary hover:bg-gray-50 dark:hover:bg-gray-700'
                    }`}
                  >
    ...
    <div className="fixed inset-0 z-[99999] flex items-end sm:items-center justify-center">
      ...
      <div className="relative w-full sm:w-[480px] max-h-[85vh] sm:max-h-[90vh] bg-white dark:bg-gray-900 rounded-t-2xl sm:rounded-2xl shadow-2xl flex flex-col animate-slideUp sm:animate-fadeIn overflow-hidden">
        ...
        <div className="flex items-center justify-between px-5 py-4 border-b border-gray-100 dark:border-gray-800">
          <div className={`flex items-center gap-1 font-poppins ${tabFontClass}`}>
            {tabs.map((tab, idx) => (
