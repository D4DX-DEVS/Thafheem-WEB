...
const DragDropQuiz = () => {
  const { translationLanguage, quranFont } = useTheme();
  ...
  // Get drag & drop title based on language
  const getDragDropTitle = () => {
    if (translationLanguage === 'hi') return 'खींचें और छोड़ें';
    if (translationLanguage === 'ur') return 'کھینچیں اور چھوڑیں';
    if (translationLanguage === 'bn') return 'টেনে এনে ছেড়ে দিন';
    if (translationLanguage === 'ta') return 'இழுத்து விடுங்கள்';
    if (translationLanguage === 'mal') return 'ഡ്രാഗ് & ഡ്രോപ്';
    return 'Drag and Drop';
  };

  // Get "Ayah" label based on language
  const getAyahLabel = () => {
    if (translationLanguage === 'mal') return 'ആയത്ത്';
    if (translationLanguage === 'ur') return 'آیت';
    if (translationLanguage === 'hi') return 'आयत';
    if (translationLanguage === 'bn') return 'আয়াত';
    if (translationLanguage === 'ta') return 'வசனம்';
    return 'Ayah';
  };

  // Convert decimal digit string to Eastern Arabic numerals
  const toArabicNumeral = (n) => String(n).replace(/[0-9]/g, d => '٠١٢٣٤٥٦٧٨٩'[d]);
        ...
        if (wordsArray && Array.isArray(wordsArray) && wordsArray.length > 0) {
          ...
          const validWords = wordsArray
            .map((word) => {
              ...
              // Extract meaning based on language — use correct column per language
              let meaning = '';
              if (translationLanguage === 'mal') {
                meaning = word.MalMeaning || word.WordMeaning || '';
              } else if (translationLanguage === 'E' || translationLanguage === 'en') {
                meaning = word.EngMeaning || word.WordMeaning || '';
              } else {
                // Urdu, Hindi, Bangla, Tamil — all use WordMeaning column
            ...
            .filter(
              (word) =>
                ...
                word.Meaning &&
                word.Meaning.trim() !== "" &&
                !word.WordPhrase.includes("﴿") && // Filter out verse numbers in Arabic
                !word.WordPhrase.trim().match(/^[\u0660-\u0669\u06F0-\u06F9\d\s]+$/) && // Filter Arabic/Hindi numerals only
                !word.Meaning.trim().match(/^[﴿﴾\u0660-\u0669\u06F0-\u06F9\(\)\d\s]+$/) // Filter verse-number meanings e.g. (١)
            );
    ...
    return (
      <div className="min-h-screen bg-gray-100 flex items-center justify-center px-4">
        <div className="fixed inset-0 bg-gray-500 bg-opacity-50 flex items-center justify-center z-50 p-4">
          <div className="bg-white dark:bg-[#1C1C1E] rounded-lg shadow-xl p-4 sm:p-6 w-full max-w-sm sm:max-w-md">
        ...
        <div className="max-w-6xl mx-auto px-3 sm:px-4 py-2">
          <div className="flex items-center justify-between">
            ...
            >
              ...
              {/* Dropdown Menu */}
              {showSurahSelector && (
                <div className="absolute top-full left-0 mt-1 w-80 bg-white dark:bg-[#1C1C1E] rounded-lg shadow-xl border border-gray-200 dark:border-gray-700 z-50">
                  {/* Header */}
        ...
        <div className="mx-auto px-3 sm:px-4 py-3">
          <div className="flex flex-col sm:flex-row items-start sm:items-center justify-between gap-2 sm:gap-0">
            <div className="flex items-center gap-25">
              <h1
                className={`text-lg sm:text-xl font-semibold ${getTranslationFontClass()}`}
                style={{ color: "#2AA0BF" }}
              >
                {getDragDropTitle()}
              </h1>
            ...
            <div className="hidden sm:block text-xs sm:text-sm text-gray-600 dark:text-white">
              <span>{getAyahLabel()}: {currentAyah} <span dir="ltr">&#xFD3E;{toArabicNumeral(currentAyah)}&#xFD3F;</span></span>
              {totalPages > 1 && (
                <span className="ml-4">
      ...
      <div className="w-full mx-auto px-3 sm:px-4 py-4 sm:py-6 dark:bg-gray-900">
        <div className="max-w-6xl mx-auto">
          <div className="bg-gray-50 dark:bg-gray-900 rounded-lg p-3 sm:p-6">
            {loadingWordMeanings && <WordGridSkeleton count={8} />}
                  ...
                  <div className="flex flex-row gap-2 sm:gap-3 lg:gap-4 justify-center">
                    <div className="flex flex-col gap-2 sm:gap-3 lg:gap-4 order-1">
                      ...
                      {currentArabicTexts.map((item) => (
                        <div
                          key={item.id}
                          className="bg-[#EBEEF0] dark:bg-[#252525] dark:text-white p-3 sm:p-4 rounded text-center w-[164px] sm:w-48 lg:w-50 h-10 sm:h-12 flex items-center justify-center"
                        >
                          <span
                          ...
                          <div
                            ...
                            className={`w-full ${getTranslationFontClass()} sm:w-56 lg:w-64 h-10 sm:h-12 border rounded-lg flex items-center justify-center text-xs sm:text-sm transition-colors ${
                              droppedItem
                                ...
                                : "border-gray-300 bg-[#EBEEF0] hover:border-blue-300 dark:bg-[#252525]"
                            }`}
