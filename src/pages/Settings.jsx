...
const Settings = ({ onClose }) => {
  const {
    ...
    tajweedEnabled: contextTajweedEnabled,
    setTajweedEnabled: setContextTajweedEnabled,
  ...
  const effectiveFontSize = translationFontSize;
  const [viewType, setViewType] = useState(contextViewType);
  const [tajweedEnabled, setTajweedEnabled] = useState(contextTajweedEnabled);
  const [playbackSpeed, setPlaybackSpeed] = useState(contextPlaybackSpeed);
  const [reciter, setReciter] = useState(contextReciter);

  // Sync from context when changed externally (sidebar, StickyAudioPlayer)
  useEffect(() => {
    setPlaybackSpeed(contextPlaybackSpeed);
  }, [contextPlaybackSpeed]);

  useEffect(() => {
    setReciter(contextReciter);
  }, [contextReciter]);
  ...
  const handleReset = () => {
    ...
    setQuranFont("ScheherazadeNew-Regular");
    setContextQuranFont("ScheherazadeNew-Regular");
    setFontSize(26);
    setContextFontSize(26);
    setLanguage("Malayalam");
    setContextTranslationLanguage("mal");
    setTranslationFontSize(18);
    setContextTranslationFontSize(18);
    setViewType("Ayah Wise");
    setContextViewType("Ayah Wise");
        ...
        <div className="flex-1 overflow-y-auto p-5 space-y-6">
            ...
            <div className="relative">
              <select
                ...
                onChange={(e) => {
                  ...
                  if (analytics) {
                    const fromCode = contextTranslationLanguage;
                    const toCode = selectedCode === 'E' ? 'en' : selectedCode;
          ...
          <div className="space-y-3">
            <div className="flex justify-between items-center">
              ...
              <span className="text-sm font-semibold text-gray-900 dark:text-white">{effectiveFontSize}px</span>
            </div>
            <div className="flex items-center gap-4">
              <button
              ...
              <div className="flex-1 h-2 bg-gray-100 dark:bg-gray-800 rounded-full overflow-hidden">
                <div
                  className="h-full bg-primary rounded-full"
                  style={{ width: `${((effectiveFontSize - 12) / (22 - 12)) * 100}%` }}
                />
              </div>
              <button
                onClick={() => setTranslationFontSize(Math.min(22, translationFontSize + 1))}
                className="p-2 rounded-lg border border-gray-200 dark:border-gray-700 hover:bg-gray-50 dark:hover:bg-gray-800 text-gray-600 dark:text-gray-300 transition-colors"
              >
                <Plus className="w-5 h-5" />
              </button>
          ...
          {/* Quran Display */}
          <div className="space-y-3">
            <h3 className="text-sm font-medium text-gray-500 dark:text-gray-400 uppercase tracking-wider">Quran Display</h3>
            <div className="flex items-center justify-between p-3 bg-gray-50 dark:bg-gray-800/50 rounded-xl border border-gray-100 dark:border-gray-700">
              <div>
                <span className="text-sm font-medium text-gray-700 dark:text-gray-300">Colour coded Tajweed</span>
                <p className="text-xs text-gray-400 dark:text-gray-500 mt-0.5">Tajweed coloured Mus'haf</p>
              </div>
              <button
                onClick={() => {
                  const next = !tajweedEnabled;
                  setTajweedEnabled(next);
                  setContextTajweedEnabled(next);
                }}
                className={`relative inline-flex h-6 w-11 items-center rounded-full transition-colors focus:outline-none ${tajweedEnabled ? "bg-primary" : "bg-gray-300 dark:bg-gray-600"}`}
              >
                <span
                  className={`inline-block h-4 w-4 transform rounded-full bg-white transition-transform ${tajweedEnabled ? "translate-x-6" : "translate-x-1"}`}
                />
              </button>
            </div>
          </div>
          ...
          <div className="space-y-3">
            ...
            <div className="space-y-2">
              <span className="text-xs text-gray-500 dark:text-gray-400">Audio Types</span>
              {[
                { value: 'quran', label: 'Quran Audio', icon: '🕌' },
                { value: 'translation', label: 'Translation Audio', icon: '📖' },
                { value: 'interpretation', label: 'Interpretation Audio', icon: '📝' },
              ].filter(type => !location.pathname.startsWith('/blockwise') || type.value === 'quran').map((type) => {
                const available = getAvailableAudioTypes(contextTranslationLanguage);
                const isAvailable = available.includes(type.value);
                const isSelected = contextAudioTypes.includes(type.value);
                return (
                  <button
                    key={type.value}
                    onClick={() => {
                      ...
                      const toggled = isSelected
                        ? contextAudioTypes.filter(t => t !== type.value)
                        : [...contextAudioTypes, type.value];
                      const canonicalOrder = ['quran', 'translation', 'interpretation'];
                      const newTypes = canonicalOrder.filter(t => toggled.includes(t));
                      if (newTypes.length > 0) {
                        setContextAudioTypes(newTypes);
                      }
                    }}
                    ...
                    className={`w-full flex items-center gap-3 px-3 py-2.5 rounded-xl transition-all text-left ${
                      !isAvailable
                        ...
                        : 'bg-gray-50 dark:bg-gray-800/50 border border-gray-200 dark:border-gray-700 hover:border-primary/50'
                    }`}
                  >
                    <span className="text-base">{type.icon}</span>
                    <span className={`text-sm flex-1 ${isSelected ? 'text-primary font-medium' : 'text-gray-700 dark:text-gray-300'}`}>
                      {type.label}
                    </span>
                    ...
                    {!isAvailable && (
                      <span className="text-[10px] text-gray-400 dark:text-gray-500">Not available</span>
                    )}
                  </button>
                );
              })}
              {!location.pathname.startsWith('/blockwise') && !['mal', 'ur'].includes(contextTranslationLanguage) && (
                <p className="text-[10px] text-gray-400 dark:text-gray-500 px-1">
                  Translation & Interpretation audio available for Malayalam and Urdu only
                </p>
              )}
            </div>
            ...
            <div className="space-y-2">
              ...
              <input
                ...
                onChange={(e) => {
                  ...
                  setContextPlaybackSpeed(newSpeed);
                }}
