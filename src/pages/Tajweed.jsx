...
import NavigateSurah from "./NavigateSurah";
import {
...
// Process sub-rules data
          ...
          if (Array.isArray(subRulesData) && subRulesData.length > 0) {
            // Transform sub-rules to consistent format, filtering out the parent rule itself
            processedSubRules = subRulesData
              .filter((subRule) => subRule.RuleNo !== ruleId.toString())
              .map((subRule) => ({
              id: subRule.RuleNo,
              title: subRule.Rule,
...
// Process nested sub-rules data
          ...
          if (
            ...
            // Transform nested sub-rules to consistent format, filtering out the parent sub-rule itself
            processedNestedSubRules = nestedSubRulesData
              .filter((nestedSubRule) => nestedSubRule.RuleNo !== subRuleId.toString())
              .map(
              (nestedSubRule) => ({
    ...
    <div className="min-h-screen bg-white dark:bg-gray-900">
        ...
        <div className="max-w-6xl mx-auto px-2 sm:px-4 py-3 sm:py-4 border-b border-gray-200">
          <div className="flex items-center gap-2 sm:gap-3">
            ...
            <button
              ...
              className="ml-auto flex items-center gap-1.5 px-3 py-1.5 rounded-lg bg-[#2AA0BF] text-white text-xs sm:text-sm font-medium hover:bg-[#2190AF] transition-colors whitespace-nowrap flex-shrink-0"
              title="Open Reading page with Tajweed mode enabled"
            >
              <span className="font-malayalam">തജ്‌വീദ് മുസ്ഹഫ്</span>
              <ArrowRight className="w-3.5 h-3.5 flex-shrink-0" />
            </button>

            {/* Surah selector portal */}
            {showSurahSelector && portalRoot && createPortal(
              <div className="fixed inset-0 z-[99990]">
                <NavigateSurah
                  onClose={() => setShowSurahSelector(false)}
                  onSurahSelect={(surah) => {
                    const surahId = surah?.number ?? surah?.id;
                    if (surahId) {
          ...
          <div className="space-y-4 sm:space-y-6">
            ...
            {tajweedRules.length > 0
              ? tajweedRules.map((rule, index) => (
                  <div
                    key={rule.id || index}
                    className="border border-gray-200 rounded-lg dark:bg-[#1C1C1E]"
                  >
                    <button
                    ...
                    >
                      <div className="flex items-center gap-2 pr-2">
                        <h3
                          className="text-sm sm:text-base font-medium text-[#2AA0BF] dark:text-[#2AA0BF] font-malayalam flex items-center gap-1"
                        >
                          {rule.title}
                        </h3>
                        {/* {rule.hasSubRules && (
      ...
      {isPopupOpen && (
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/40 backdrop-blur-sm px-2 sm:px-4">
          <div className="bg-white dark:bg-gray-900 rounded-2xl shadow-2xl max-w-xl w-full max-h-[90vh] flex flex-col overflow-hidden border border-gray-200 dark:border-gray-700 relative">
            {/* Close button */}
            <button
              onClick={() => {
            ...
            <div className="overflow-y-auto flex-1 p-4 sm:p-6 space-y-4 sm:space-y-6">
              ...
              <div className="bg-gradient-to-br from-gray-200 to-gray-300 dark:from-gray-800 dark:to-gray-900 rounded-xl sm:rounded-2xl p-4 sm:p-5 shadow-inner">
                <div className="w-full flex justify-center items-center">
                  {/* Arabic Text Display */}
                  <div className="bg-white/50 dark:bg-gray-800/50 backdrop-blur-sm rounded-lg p-3 sm:p-4 w-full max-w-[406px] max-h-[45vh] overflow-y-auto flex flex-col justify-center shadow-inner border border-gray-300/30 dark:border-gray-700/30">
                    {loadingArabicText ? (
                      ...
                      <div className="text-center w-full px-2">
                        {/* When a verse key is available, always show tajweed color-coded images */}
                        {currentVerseKey && /^\d+:\d+$/.test(currentVerseKey) ? (
                          <div
                            dir="rtl"
                            className="flex flex-wrap justify-center items-center"
                            style={{ lineHeight: '2.5', minHeight: '60px' }}
                          >
                            <TajweedVerse
                              suraid={parseInt(currentVerseKey.split(':')[0], 10)}
                              ayaid={parseInt(currentVerseKey.split(':')[1], 10)}
                              fontSize={36}
                              tooltip={false}
                            />
                          </div>
