...
const Verse = ({ onClose }) => {
  ...
  useEffect(() => {
    ...
    const load = async () => {
      try {
        setLoading(true);
        setError(null);
        const data = await listSurahVerseIndex(translationLanguage);
        if (!isCancelled) {
          setVerses(
            data.map((v) => ({ 
              id: v.id, 
  ...
  return (
    <div className="bg-white dark:bg-[#1C1C1E] w-full  h-full max-w-md  flex flex-col  rounded-lg font-poppins">
      {/* Search Bar */}
    ...
    <ul className="space-y-2">
      {filteredVerses.map((item, index) => (
        ...
        >
          ...
          {/* Surah */}
          <span className={`flex-1 truncate ${surahFontClass}`}>{item.surah}</span>
