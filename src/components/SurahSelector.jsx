...
const SurahSelector = ({ isOpen, onClose, onSelect, currentSurahId }) => {
    ...
    const loadData = async () => {
      try {
        ...
        const [surahsData, rangesData] = await Promise.all([
          fetchSurahs({ language: translationLanguage }),
          fetchPageRanges()
        ]);
  ...
  return (
    <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50 p-4">
      <div className="bg-white dark:bg-[#1C1C1E] rounded-lg shadow-xl w-full max-w-md max-h-[80vh] overflow-hidden">
        {/* Header */}
