...
const BlockInterpretationModal = ({
  ...
  const [showVerseRefModal, setShowVerseRefModal] = useState(false);
  const [verseRefTarget, setVerseRefTarget] = useState({ surahId: null, ayah: null });
  ...
  const handleNoteHighlightClick = (e) => {
    ...
    if (type === "verse" || type === "range") {
      // Handle verse/range navigation — open VerseReferenceModal (Arabic + Translation)
  ...
  const handleContentClick = (e) => {
    ...
    // Check if clicked element is a Malayalam interpretation cross-reference (S:N in interpretation text)
    const interpCrossref = target.closest('.mal-interpretation-crossref-link');
    if (interpCrossref) {
      e.preventDefault();
      e.stopPropagation();
      const crossrefSurah = parseInt(interpCrossref.getAttribute('data-crossref-surah'), 10);
      const crossrefNo = parseInt(interpCrossref.getAttribute('data-crossref-no'), 10);
      if (crossrefSurah && crossrefNo) {
        navigateToMalInterpretation(crossrefSurah, crossrefNo);
      }
      return;
    }

    // Check if clicked element is a verse reference link
    // Opens VerseReferenceModal (Arabic + Translation with clickable interpretation numbers)
    // Replicates mobile app flow: verse ref → show block translation → tap number → interpretation
    ...
    if (verseLink) {
      ...
      if (surah && ayah) {
        setVerseRefTarget({ surahId: parseInt(surah, 10), ayah: parseInt(ayah, 10) });
        setShowVerseRefModal(true);
        return;
      }
    }
  ...
  // Navigate the current modal to a Malayalam interpretation by surah + InterpretationNo.
  // Matches old site thaf-api/intptrayarange/:surah/:interpretationNo/M behavior.
  const navigateToMalInterpretation = async (surah, interpretationNo) => {
    try {
      const items = await fetchInterpretationRange(surah, '1', interpretationNo, 'mal');
      if (!items || items.length === 0) return;
      const item = items[0];
      const fromAyah = item.verseId || 1;
      const toAyah = item.toAyah || fromAyah;
      const range = fromAyah === toAyah ? String(fromAyah) : `${fromAyah}-${toAyah}`;
      setCurrentSurahId(surah);
      setCurrentRange(range);
      setCurrentInterpretationNo(interpretationNo);
    } catch (err) {
      console.warn('[BlockInterpretationModal] ⚠️ Failed to navigate to interpretation:', err.message);
    }
  };
  ...
  const processVerseReferences = (text) => {
    ...
    let processed = text.replace(versePattern, (match, surah, ayah) => {
      ...
      if (match.includes('verse-reference-link') || match.includes('mal-interpretation-crossref-link')) {
        return match;
      }

      // In Malayalam, (S:N) cross-references in interpretation text point to
      // interpretation #N of Surah S — same as old site intptrayarange/:surah/:interpretationNo
      if (currentLanguage === 'mal' && isInterpretationRef(match)) {
        return `<span class="mal-interpretation-crossref-link inline-block cursor-pointer text-cyan-500 hover:text-cyan-600 dark:text-cyan-400 dark:hover:text-cyan-300 underline decoration-cyan-500/50 hover:decoration-cyan-600 dark:decoration-cyan-400/50 dark:hover:decoration-cyan-300 transition-colors" data-crossref-surah="${surah}" data-crossref-no="${ayah}" title="Click to view interpretation ${surah}:${ayah}">${match}</span>`;
      }
  ...
  return createPortal(
    <>
      <style>
        ...
        /* Malayalam interpretation styling */
        .font-malayalam.interpretation-content,
        .font-malayalam .interpretation-content,
        .font-malayalam .interpretation-content p,
        .font-malayalam .interpretation-content span,
        .font-malayalam .interpretation-content a,
        .font-malayalam .interpretation-content * {
      ...
      <div className="fixed inset-0 z-[99999] flex items-end sm:items-center justify-center"
        onTouchMove={(e) => {
          // Prevent background scroll on touch when touching backdrop
          const modalContent = e.currentTarget.querySelector('[data-modal-content]');
      ...
      >
        ...
        <div
          ...
          onClick={handleClose}
          style={{ pointerEvents: isClosing ? 'none' : 'auto', touchAction: 'none' }}
        />
        ...
        >
          ...
          <div 
            ...
            } overscroll-contain px-4 sm:px-6 py-6 sm:py-8 ${currentLanguage === 'E' || currentLanguage === 'en' ? '' : 'min-h-0'}`}
            style={{
              overflowX: 'hidden',
          ...
          >
            ...
            {/* Interpretation Content */}
            <div className={`space-y-6 sm:space-y-8 ${currentLanguage === 'mal' ? 'font-malayalam' : 'font-poppins'}`} key={`block-${currentSurahId}-${currentRange}-${currentInterpretationNo}`}>
              {content.map((item, idx) => {
                const isUrdu = currentLanguage === 'ur' || currentLanguage === 'urdu';
                const isMalayalam = currentLanguage === 'mal';
                return (
                  <div
                  ...
                  >
                    <div
                      className={`interpretation-content text-gray-700 leading-relaxed dark:text-gray-300 text-sm sm:text-base prose dark:prose-invert max-w-none text-justify whitespace-pre-line ${isUrdu ? 'font-urdu-nastaliq urdu-interpretation-content' : ''} ${isMalayalam ? 'font-malayalam' : ''}`}
      ...
      {/* Verse Reference Modal (Arabic + Translation with clickable interpretation numbers) */}
      {showVerseRefModal && verseRefTarget.surahId && verseRefTarget.ayah && (
        <VerseReferenceModal
          surahId={verseRefTarget.surahId}
          ayah={verseRefTarget.ayah}
          language={currentLanguage}
          onInterpretationClick={(surah, interpretationNo) => {
            setShowVerseRefModal(false);
            navigateToMalInterpretation(surah, interpretationNo);
          }}
          onClose={() => setShowVerseRefModal(false)}
        />
      )}
