...
const PlayAudio = ({ audioSrc, title, onClose, autoPlay = false }) => {
  ...
  const { trackUsage, startTimer, trackDuration } = useTrackFeature('audio-player');
  ...
  useEffect(() => {
    ...
    // Don't clear media session on re-render — only on unmount
  }, [title, onClose]);

  // Clear media session only on unmount
  useEffect(() => {
  ...
  useEffect(() => {
    if (duration > 0) {
      ...
      if (now - lastPositionUpdateRef.current >= 1000) {
        ...
        updatePositionState({ duration, position: currentTime });
      }
    }
