  ...
  const handleBookmark = async () => {
    ...
    try {
      ...
      if (isBookmarked) {
        ...
        // Add bookmark - fetch localized surah name for the current language
        let surahName = surahInfo?.name || surahInfo?.arabic || `Surah ${surahId}`;
        try {
          ...
          if (localizedSurah?.name) surahName = localizedSurah.name;
        } catch (_) {
          surahName = surahInfo?.name || surahInfo?.arabic || `Surah ${surahId}`;
        }
        ...
        <div className="flex flex-wrap sm:flex-nowrap items-center gap-2 sm:space-x-3 pr-12 sm:pr-0">
          ...
          <div className="relative">
            <button
              ...
              className="flex font-poppins items-center space-x-2 px-3 sm:px-4 py-2 bg-gray-100 dark:bg-[#252525] dark:text-white hover:bg-gray-200 dark:hover:bg-gray-600 rounded-lg text-xs sm:text-sm font-medium text-gray-700 transition-colors"
            >
            ...
            {isDropdownOpen && !loading && (
              <div className="absolute sm:text-sm text-sm top-full left-0 mt-2 bg-white dark:bg-[#1C1C1E] shadow-lg rounded-lg overflow-auto w-80 sm:w-45 h-[calc(100vh-100px)] z-50">
                {allSurahs.map((surah) => (
                  <div
                    key={surah.number}
          ...
          <div className="relative">
            ...
            {verseDropdownOpen && (
              <div className="absolute top-full left-0 mt-2 bg-white dark:bg-[#1C1C1E] shadow-lg rounded-lg overflow-auto w-auto max-h-60 z-50">
