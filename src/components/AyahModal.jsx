    ...
    const loadVerseData = async () => {
      ...
      try {
        ...
        const translationPromise = (translationLanguage === 'ta' || translationLanguage === 'hi' || translationLanguage === 'bn' || translationLanguage === 'E' || translationLanguage === 'ur')
          ...
          : fetchAyahAudioTranslations(activeSurahId, verseNumberForRequest).catch((err) => {
              console.warn(`[AyahModal] ⚠️ Translation fetch failed for Surah ${activeSurahId}, Ayah ${verseNumberForRequest}:`, err.message);
              return null;
            });
    ...
    <div className="fixed inset-0 z-[99999] flex items-end sm:items-center justify-center">
          ...
          {verseData && !isEnglishTranslation && translationLanguage !== 'ur' && (
            <div className="mb-6 sm:mb-8">
              {translationLanguage === 'bn' ? (
                ...
                <p
                  className="text-gray-700 leading-[1.8] font-malayalam dark:text-gray-200 px-2 sm:px-0 text-justify"
                  style={{ fontSize: `${adjustedTranslationFontSize}px` }}
                >
                  {verseData.translation}
                </p>
                ...
                {(translationLanguage === 'mal' ? interpretationData.slice(0, 1) : interpretationData).map((interpretation, index) => {
                  ...
                  return (
                    ...
                    >
                      ...
                      <div
                        className={`text-gray-700 leading-[1.8] dark:text-gray-300 text-sm sm:text-base text-justify whitespace-pre-line ${
                          translationLanguage === 'ur' ? 'font-urdu-nastaliq' : translationLanguage === 'mal' ? 'font-malayalam' : 'font-poppins'
                        }`}
                        style={{ 
      ...
      {showWordByWord && createPortal(
        <div className="fixed inset-0 bg-black/60 backdrop-blur-sm flex items-start justify-center z-[100000] pt-24 sm:pt-28 lg:pt-32 p-4 overflow-hidden">
          <div className="bg-white dark:bg-[#1C1C1E] rounded-xl shadow-2xl w-full sm:w-auto sm:max-w-6xl max-w-[95vw]">
            <WordByWord
