...
const InterpretationModal = ({ surahId, verseId, interpretationNo, language, onClose }) => {
  ...
  const processVerseReferences = (text) => {
    ...
    return text.replace(versePattern, (match, surah, ayah) => {
      ...
      // In Malayalam, (S:N) with both parentheses = interpretation cross-ref
      if (isMal && isInterpretationRef(match)) {
  ...
  const isMalayalam = effectiveLang === 'mal';

  return createPortal(
    <>
      {isMalayalam && (
        <style>{`
          .malayalam-interpretation-content,
          .malayalam-interpretation-content p,
          .malayalam-interpretation-content span,
          .malayalam-interpretation-content a,
          .malayalam-interpretation-content * {
        ...
        <div className="relative w-full sm:w-auto sm:max-w-4xl xl:max-w-[1073px] max-h-[85vh] sm:max-h-[90vh] bg-white dark:bg-gray-900 rounded-t-2xl sm:rounded-2xl shadow-2xl flex flex-col animate-slideUp sm:animate-fadeIn overflow-hidden">
          ...
          <div className="px-4 sm:px-6 py-6 sm:py-8 overflow-y-auto flex-1">
            {loading ? (
              ...
              <div className={isMalayalam ? 'font-malayalam' : 'font-poppins'}>
                {interpretationData.map((interpretation, index) => {
                  ...
                  return (
                    <div key={index} className="mb-6 sm:mb-8">
                      <div
                        className={`text-gray-700 leading-relaxed dark:text-gray-300 text-sm sm:text-base prose dark:prose-invert max-w-none text-justify ${isUrdu ? 'font-urdu-nastaliq urdu-interpretation-content' : ''} ${isMalayalam ? 'font-malayalam malayalam-interpretation-content' : ''}`}
                        style={{ 
                        ...
                        dir={isUrdu ? 'rtl' : 'ltr'}
                        onClick={(e) => {
                          // Handle clicks on Malayalam interpretation cross-references
                          const interpCrossref = e.target.closest('.mal-interpretation-crossref-link');
                          if (interpCrossref) {
                            e.preventDefault();
                            e.stopPropagation();
                            const crossrefSurah = interpCrossref.getAttribute('data-crossref-surah');
                            const crossrefNo = interpCrossref.getAttribute('data-crossref-no');
                            if (crossrefSurah && crossrefNo) {
                              onClose();
                              setTimeout(() => {
                                window.location.href = `/interpretation-blockwise?surahId=${crossrefSurah}&ipt=${crossrefNo}&lang=mal`;
                              }, 100);
                            }
                            return;
                          }
